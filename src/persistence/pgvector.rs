...
impl DatabaseAdapter for PgVectorAdapter {
    ...
    async fn create_many_raw(&self, _table: &str, _entities: Vec<serde_json::Value>) -> Result<Vec<EntityId>> {
        Ok(vec![])
    }
    ...
    async fn query_raw(&self, _table: &str, _filter: Option<Filter>, _sort: Option<Vec<Sort>>, _pagination: Option<Pagination>) -> Result<QueryResult> {
...
impl VectorAdapter for PgVectorAdapter {
    async fn store_embedding(&self, id: &EntityId, embedding: &[f32], _metadata: Option<serde_json::Value>) -> Result<()> {
        debug!(id = %id.as_str(), dims = embedding.len(), "Storing embedding (stub)");
        Ok(())
    }

    async fn search_similar(&self, embedding: &[f32], limit: usize, _filter: Option<Filter>) -> Result<Vec<(EntityId, f32, serde_json::Value)>> {
        debug!(dims = embedding.len(), limit, "Searching similar vectors (stub)");
        Ok(vec![])
    }
